...
public class KThread {
  ...
  private LinkedList<KThread> JoinedThreads;
  ...
  public KThread() {
    if (currentThread != null)
      ...
      currentThread = this;
      this.tcb = TCB.currentTCB();
      this.name = "main";
      restoreState();

      createIdleThread();
    }
    this.JoinedThreads = new LinkedList<KThread>();
  }
  ...
  public KThread setTarget(Runnable target) {
    Lib.assertTrue(this.status == statusNew);
   ...
   * the <tt>fork</tt> method) and the other thread (which executes its target's
   * <tt>run</tt> method).
   */
  public void fork() {
    Lib.assertTrue(this.status == statusNew);
    Lib.assertTrue(this.target != null);

    Lib.debug(dbgThread, "Forking thread: " + toString() + " Runnable: " + this.target);

    boolean intStatus = Machine.interrupt().disable();

    this.tcb.start(new Runnable() {
      @Override
      public void run() {
        runThread();
      }
   ...
   * to do so. This method is automatically called when a thread's <tt>run</tt>
  ...
  public static void finish() {
    ...
    // if the current thread has any joined threads
    // then they must be removed
    while (!KThread.currentThread.JoinedThreads.isEmpty()) {
      KThread t = KThread.currentThread.JoinedThreads.remove();
      t.status = statusReady;
      KThread.readyQueue.waitForAccess(t);
    }
  ...
  public void ready() {
    ...
    Lib.assertTrue(Machine.interrupt().disabled());
    Lib.assertTrue(this.status != statusReady);
  ...
  public void join() {
    ...
    if (this.status != statusFinished)
      this.JoinedThreads.add(KThread.currentThread);
    Machine.interrupt().restore(intStatus);
  }
   ...
   * switch to the new thread by calling <tt>TCB.contextSwitch()</tt>, and load
   * the state of the new thread. The new thread becomes the current thread.
   *
   * <p>
  ...
  protected void restoreState() {
    ...
    Lib.assertTrue(Machine.interrupt().disabled());
    Lib.assertTrue(this == currentThread);
    Lib.assertTrue(this.tcb == TCB.currentTCB());
  ...
  private static class PingTest implements Runnable {
    ...
    public void run() {
      for (int i = 0; i < 5; i++) {
        System.out.println("*** thread " + this.which + " looped " + i + " times");
        currentThread.yield();
      }
    }
  ...
  public static void selfTest() {
    ...
    KThread slave1 = new KThread(new SlaveThread("slave1"));
    slave1.setName("slave1");
    slave1.fork();
    slave1.join();

    new MasterThread("master1", slave1).run();
  }

  /*
   * A MasterThread waits for its SlaveThread to complete
   */
  private static class MasterThread implements Runnable {
    String name;
    KThread slave;

    public MasterThread(String name, KThread slave) {
      this.name = name;
      this.slave = slave;
      Lib.debug(dbgThread, Machine.timer().getTime() + " " + this.name + " -- created");
    }

    @Override
    public void run() {
      Lib.debug(dbgThread, Machine.timer().getTime() + " " + this.name + " -- started");
      this.slave.join();
      for (int i = 0; i < 5; i++) {
        System.out.println(Machine.timer().getTime() + " " + this.name + " -- MasterThread working (" + i + ")");
        currentThread.yield();
      }
      Lib.debug(dbgThread, Machine.timer().getTime() + " " + this.name + " -- ended");
      KThread.finish();
    }
  ...
  private static class SlaveThread implements Runnable {
    ...
    public void run() {
      ...
      for (char c = 65; c < 91; c++) {
        System.out.println(Machine.timer().getTime() + " " + this.name + " -- SlaveThread working (" + c + ")");
        currentThread.yield();
      }
      Lib.debug(dbgThread, Machine.timer().getTime() + " " + this.name + " -- ended");
      KThread.finish();
   ...
   * ready (on the ready queue but not running), running, or blocked (not on the
   * ready queue and not running).
   */
